...
- Awkwardness using `guard_msgs`: From what I can understand, the way `guard_msgs` works is that it asserts that the message printed by the following command is the same as the one in the comment. However, it also seems to care for white space, which can be extremely annoying especially with lengthy terms that are printed in seperate lines in the info-view. Ideally there'd be a way to specify that the actual _terms_ match instead of just looking at the white-space. (Ithink this can be done in traditional Lean using a temporary lemma where the proof is fixed to just be `rfl`, but that approach also pollutes the namespace.)
